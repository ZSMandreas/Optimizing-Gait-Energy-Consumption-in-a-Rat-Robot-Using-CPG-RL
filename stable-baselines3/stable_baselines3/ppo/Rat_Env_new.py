...
class Go2Env(gym.Env):
    def __init__(
        ...
        # 这里暂时不把 command 拼进 obs（如果要加，可以再扩展）
        final_low = final_low_base
        final_high = final_high_base
    ...
    def step(self, action):
        ...
        """
        # print("Original action in step():", action)
        self.cpgstate = []
        joint_targets = []
        E_outer = 0.0   # 当前 RL step 的能量
        mapping_action = action.copy()
        # 这一 step 开始时 COM y
        ...
        # print("Mapped action:", mapping_action)
        # print("Original action:", action)
        # 下发控制
        self.data.ctrl[:self.action_dim] = mapping_action
        # print("Ctrl in sim step:", self.data.ctrl[:self.action_dim])
        ...
        # 原始状态 [obs, action, next_obs]
        state = self.state.get_state(mapping_action)
        ...
        # ====== q_err: 目标关节角 - 实际关节角 ======
        # 1) 目标关节角：action 前 8 维就是 4 条腿的关节角
        ...
        # 记录动作
        reward_info["action"] = action[:8].astype(float)
    ...
    def calculate_rewards(self, state, action):
        ...
        r_vx = np.exp(-((v_bx - self.v_bx_desired) ** 2) / 0.00130) - 1
...
#         # ------------------- command velocities (随机小范围) -------------------
#         # v_by_desired 在 [-0.14, -0.10] 内随机，每隔一段时间重采样一次
#         self.v_by_min = -0.14
#         self.v_by_max = -0.10
#         self.v_by_desired = -0.12  # 会在 reset 和后续定期重采样
#         self.v_bx_desired = 0.0
#         self.w_bz_desired = 0.0

#         # 每隔多少个 env.step() 重新采样一次指令速度
#         self.cmd_update_interval = 50
#         self.cmd_step_counter = 0
#         # ---------------------------------------------------------
...
#         # 能量（原 8 个腿部电机的 dof 索引）
#         qfrc_idx = [4, 3, 8, 7, 17, 16, 21, 20]
#         self.energy_meter = EnergyMeter(
#             self.model,
#             motor_dof_idx=qfrc_idx,
#             mass=self.total_mass,
#             forward_dof=1,  # 前进在 y 轴
#         )
...
#         self.E_ema = None
#         self.beta = 0.99
#         self.w_energy = 0.01  # 能量惩罚权重（可以调）
#         self.c_max = 10.0     # CoT 截断，防止爆炸
...
#         # ================= 行为维度拆分（腿部 latent + 脊柱直接控制） =================
#         self.leg_action_dim = self.n_legs * 2  # 4 条腿 * (a1, a2)
#         self.spine_action_dim = 1              # 脊柱 1 维
#         self.action_dim = self.leg_action_dim + self.spine_action_dim  # 8 + 1 = 9
#         # ==============================================================================

#         self.cpgs = [OscillatorLeg() for _ in range(self.n_legs)]
#         self.cpgstate = []
#         self.last_action = np.zeros(self.action_dim)  # 9 维
...
#         # ================= 行为范围（前 8 维 latent ∈[-1,1]，第 9 维 spine ∈[-1.57,1.57]） =================
#         self.motor_ranges = [(-1.0, 1.0) for _ in range(self.leg_action_dim)] + [(-1.57, 1.57)]
#         low_action = np.array([m[0] for m in self.motor_ranges], dtype=np.float32)
#         high_action = np.array([m[1] for m in self.motor_ranges], dtype=np.float32)
#         # =======================================================================================================
...
#         # 拼接扩展后的上下界： [obs, action, next_obs]
#         final_low_base = np.concatenate((low_obs, low_action, low_obs))
#         final_high_base = np.concatenate((high_obs, high_action, high_obs))

#         # 为命令速度预留 3 维观测： [v_by_desired, v_bx_desired, w_bz_desired]
#         cmd_low = np.array([-2.0, -2.0, -2.0], dtype=np.float32)
#         cmd_high = np.array([2.0, 2.0, 2.0], dtype=np.float32)

#         final_low = np.concatenate((final_low_base, cmd_low))
#         final_high = np.concatenate((final_high_base, cmd_high))
...
#         """在给定范围里重新采样 v_by_desired。"""
#         self.v_by_desired = np.random.uniform(self.v_by_min, self.v_by_max)
#         self.v_bx_desired = 0.0
#         self.w_bz_desired = 0.0
#     # -------------------------------------------------
...
#         # reset 时先随机一个本 episode 的初始 command
...
#         state = np.concatenate([st, self.last_action, st_, cmd_obs])
#         return state, {}
...
#         # 计步 & 定期重新采样指令速度
#         self.cmd_step_counter += 1
#         if self.cmd_step_counter % self.cmd_update_interval == 0:
#             self._sample_command()
...
#         # 腿部 8 维 latent → (Fy,Fz) → 逆解到关节角
#         for i in range(self.n_legs):
#             a1 = float(action[i * 2])
#             a2 = float(action[i * 2 + 1])
#             Fy, Fz = self.mapper.map_box_area_uniform(a1, a2)
#             qVal = self.legs[i].pos_2_angle(np.array([Fy]), np.array([Fz]))  # 2 关节角
#             action[i * 2: i * 2 + 2] = qVal

#         # 下发控制（前 8 个为腿部关节角，第 9 个为 spine）
...
#             dtype=np.float32,
#         )
#         state = np.concatenate([state, cmd_obs])

#         # 轨迹记录（command 在最后，前面的索引不变）
#         self.base_velocity.append(state[24])
#         self.real_pos.append(state[:8])
#         self.f_condition.append(state[12:16])
#         self.state.foot_trajectory()
...
#         reward, reward_info = self.calculate_rewards(state, action, E_outer)
...
#     def calculate_rewards(self, state, action, E_outer):
#         """
#         计算奖励：
#         - 速度跟踪 / 稳定性
#         - 基于当前 step 的 CoT 风格能量惩罚
#         """
#         v_by = self.state.get_body_velocity()[1]
#         v_bx = self.state.get_body_velocity()[0]
#         v_bz = self.state.get_body_velocity()[2]
...
#         # --- 基于本 step 的 CoT 风格能量惩罚 ---
#         dt_total = self.model.opt.timestep * self.n
#         step_dist = abs(v_by) * dt_total
#         if step_dist > 1e-5:
#             cot_step = E_outer / (self.total_mass * 9.81 * step_dist)
#             cot_step = np.clip(cot_step, 0.0, self.c_max)
#             r_energy = -self.w_energy * cot_step
#         else:
#             cot_step = 0.0
#             r_energy = 0.0
