...
class MouseController(object):
	...
	def __init__(self, fre, time_step, spine_angle, b_scale=1.0, a_scale=1.0, duty=0.5):
		super(MouseController, self).__init__()
		PI = np.pi
		self.curStep = 0.0# Spine
		...
		self.pathStore = LegPath(b_scale=b_scale, a_scale=a_scale)
		# [LF, RF, LH, RH]
		# --------------------------------------------------------------------- #
		...
		self.period = 2 * (1 - duty)
		self.fre_cyc = fre#1.25#0.80
		self.SteNum = int(1/(time_step*self.fre_cyc))
		print("----> ", self.SteNum)
		print(f"period={self.period:.4f} (duty={duty:.2f}, swing:stance={1-duty:.2f}:{duty:.2f})")
		print(f"para_FU={self.pathStore.para_FU}")
		print(f"para_FD={self.pathStore.para_FD}")
		self.spinePhase = self.phaseDiff[3]
		# --------------------------------------------------------------------- #
	...
	def getLegCtrl(self, leg_M, curStep, leg_ID):
		curStep = float(curStep) % float(self.SteNum)
		turnAngle = self.turn_F
		leg_flag = "F"
		if leg_ID > 1:
			leg_flag = "H"
			turnAngle = self.turn_H
		radian = 2*np.pi * curStep/float(self.SteNum)
		#currentPos = self.pathStore.getRectangle(radian, leg_flag)
		currentPos = self.pathStore.getPathPoint(radian, leg_flag, self.period)
		trg_x = currentPos[0]
		trg_y = currentPos[1]
	...
	def runStep(self):
		...
		#spine = 0
		self.curStep = self.pathStore.advance_step(self.curStep, self.SteNum, self.time_step)
		...
		contact_states = []
		for i in range(4):
			phase_step = self.curStep + self.stepDiff[i]
			radian = 2 * np.pi * (float(phase_step) % float(self.SteNum)) / float(self.SteNum)
			# 后半周期为接触
			is_contact = 1 if radian >= self.period * np.pi else 0
