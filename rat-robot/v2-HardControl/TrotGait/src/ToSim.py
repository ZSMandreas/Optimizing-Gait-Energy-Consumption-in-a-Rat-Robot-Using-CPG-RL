...
class SimModel(object):
    ...
    def __init__(self, modelPath: str):
        ...
        # closure accumulators (thesis-style balance)
        self._closure_E_fric = 0.0
        self._closure_E_norm = 0.0
        self._closure_E_damp = 0.0
        self._closure_KE0 = None
        self._closure_PE0 = None
        self._closure_KE1 = None
        self._closure_PE1 = None
        self._mouse_body_id = self._name2id(mj.mjtObj.mjOBJ_BODY, "mouse")
    ...
    def runStep(self, ctrlData, cur_time_step, realtime=False, accumulate_energy=False):
        ...
        for _ in range(step_num):
            ...
            # joint damping (closure)
            self._closure_E_damp += float(0.005 * np.nansum(mot_qv * mot_qv) * sim_dt)

            # contact dissipation at feet (world vs non-world body)
            for ic in range(int(self.data.ncon)):
                ...
                mj.mj_contactForce(self.model, self.data, ic, cforce)
                f_n = float(cforce[0])
                f_t = float(np.linalg.norm(cforce[1:3]))
                v6f = np.zeros(6, dtype=np.float64)
                mj.mj_objectVelocity(self.model, self.data, mj.mjtObj.mjOBJ_BODY, foot_bid, v6f, 0)
                v_foot = v6f[3:6]
                frame = np.asarray(contact.frame, dtype=np.float64).reshape(3, 3)
                n_hat = frame[0, :]
                v_tan = v_foot - np.dot(v_foot, n_hat) * n_hat
                v_t = float(np.linalg.norm(v_tan))
                self._closure_E_fric += f_t * v_t * sim_dt
                v_n = float(np.dot(v_foot, n_hat))
                p_norm = -f_n * v_n
                if p_norm > 0.0:
                    self._closure_E_norm += p_norm * sim_dt
    ...
    def drawPath(self, show=True, save_fig_path=None):
        ...
        if show:
            ...
            plt.savefig(save_fig_path, dpi=120)
            plt.close()
        else:
            plt.close()
    ...
    def snapshot_closure_start(self) -> None:
        """Call once after initializing(), before the measurement loop."""
        from sim_closure import body_ke_pe, _mouse_mass

        mass = _mouse_mass(self.model)
        ke, pe = body_ke_pe(self.model, self.data, mass)
        self._closure_KE0 = ke
        self._closure_PE0 = pe

    def snapshot_closure_end(self) -> None:
        """Call once after the measurement loop ends."""
        from sim_closure import body_ke_pe, _mouse_mass

        mass = _mouse_mass(self.model)
        ke, pe = body_ke_pe(self.model, self.data, mass)
        self._closure_KE1 = ke
        self._closure_PE1 = pe
