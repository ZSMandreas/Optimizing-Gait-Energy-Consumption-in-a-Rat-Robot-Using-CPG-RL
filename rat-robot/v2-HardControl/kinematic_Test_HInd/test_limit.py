import numpy as np
from matplotlib import pyplot as plt
from matplotlib import animation

import math
#from foreLeg_ideal import ForeLegM
from foreLeg_limit import ForeLegM

def saveFile(points_list, file_name):
	theFile = open(file_name, 'w')

	pL = len(points_list)
	for i in range(pL):
		for j in range(2):
			theFile.write(str(points_list[i][j])+' ')
		theFile.write('\n')

	theFile.close()


if __name__ == '__main__':
	# 1: AE 	2: AB
	# 3: BC 	4: CD
	# 5: DE 	6: EF (DE+EF ~= DF)
	#leg_params = [0.031, 0.0128, 0.0118, 0.040, 0.015, 0.035]
	#leg_params = [0.031, 0.0128, 0.0118, 0.040, 0.015, 0.035, 0.015, 73*np.pi/180]

	#V1
	# 1: AE 	2: AB
	# 3: BC 	4: CD
	# 5: DE 	6: EF (DE+EF ~= DF)
	# 5: FG 	6: alpha
	#leg_params = [0.026, 0.0128, 0.0118, 0.04, 0.0175, 0.04, 0.015, 73*np.pi/180]
	#V2
	leg_params = [0.026, 0.0128, 0.0118, 0.04, 0.0175, 0.04, 0.015, 73*np.pi/180]


	fl_left = ForeLegM(leg_params)
	
	PI = math.pi
	stepNum = 100
	initRad = PI
	stepRad = (2*initRad) / stepNum
	points_list = []
	endPoints = []
	q1 = -PI#initRad
	kneePoints = []
	for i in range(stepNum):
		init_q2 = q1
		q2 = init_q2
		step_points = []
		#'''
		while q2 < PI - fl_left.limit_CBz:
			if q2 < fl_left.limit_CBz - PI:
				q2 += stepRad
				continue
			cur_points = fl_left.angel_2_pos(q1, q2)
			if len(cur_points) == 0:
				break
			step_points.append(cur_points)
			q2 += stepRad
		#'''
		q2 = init_q2
		while q2 > fl_left.limit_CBz - PI:
			if q2 > PI - fl_left.limit_CBz:
				q2 -= stepRad
				continue
			cur_points = fl_left.angel_2_pos(q1, q2)
			if len(cur_points) == 0:
				break
			step_points.append(cur_points)
			q2 -= stepRad
		#'''
		for t_points in step_points:
			endPoints.append(t_points[-2])
			kneePoints.append(t_points[-4])
		points_list.append(step_points)
		q1 += stepRad
		
	saveFile(endPoints, 'feasible_point.txt')
	saveFile(kneePoints, 'knee_point.txt')
	eL = len(endPoints)
	ey_list = [endPoints[j][0] for j in range(eL)]
	ez_list = [endPoints[j][1] for j in range(eL)]
	plt.scatter(ey_list, ez_list, s = 1, color='lightgreen')
	plt.show()

	fig, ax = plt.subplots()
	leg_list = []
	for i in range(stepNum):
		pL = len(points_list[i])
		#print(pL)
		for j in range(pL):
			leg_list.append(points_list[i][j])
		y_list = [points_list[i][j][5][0] for j in range(pL)]
		z_list = [points_list[i][j][5][1] for j in range(pL)]

		ty_list = [points_list[i][j][6][0] for j in range(pL)]
		tz_list = [points_list[i][j][6][1] for j in range(pL)]
		
		l2 = ax.scatter(ty_list, tz_list, color='lightblue')
		l1 = ax.scatter(y_list, z_list, color='lightgreen')
	ax.set_xlabel(r'$Y\ data$')
	ax.set_ylabel(r'$Z\ data$')
	ax.set_aspect(1)
	ax.set_xlim(-0.1, 0.1)
	ax.set_ylim(-0.1, 0.1)
	ax.xaxis.set_ticks(np.arange(-0.1, 0.1, step=0.02)) 
	ax.yaxis.set_ticks(np.arange(-0.1, 0.1, step=0.02))    
	ax.grid()

	lines = plt.plot([], [], 'r-',  # EA
			[], [], 'k-',			# AB
			[], [], 'r-',			# BC
			[], [], 'b-',			# CD
			[], [], 'b-',			# DE
			[], [], 'b-',			# EF
			[], [], 'b-',			# FG
			[], [], 'ko',)

	def update(k):
		ty = [leg_list[k][4][0]]
		tz = [leg_list[k][4][1]]
		for ti in range(7):
			ty.append(leg_list[k][ti][0])
			tz.append(leg_list[k][ti][1])

		for i in range(7):
			lines[i].set_data([ty[i], ty[i+1]], [tz[i], tz[i+1]])
			#lines.append(line)
		cross_p = leg_list[k][7]
		if len(cross_p) > 0:
			lines[7].set_data([cross_p[0]], [cross_p[1]])
		else:
			lines[7].set_data([], [])
		'''
		EA, = ax.plot([ty[0], ty[1]], [tz[0], tz[1]], 'red', lw=2)
		AB, = ax.plot([ty[1], ty[2]], [tz[1], tz[2]], 'black', lw=2)
		BC, = ax.plot([ty[2], ty[3]], [tz[2], tz[3]], 'red', lw=2)
		CD, = ax.plot([ty[3], ty[4]], [tz[3], tz[4]], 'g', lw=2)
		DE, = ax.plot([ty[4], ty[5]], [tz[4], tz[5]], 'g', lw=2)
		EF, = ax.plot([ty[5], ty[6]], [tz[5], tz[6]], 'g', lw=2)
		'''
		#line, = ax.plot(ty, tz, 'g--', lw=2)
		return lines

	ani = animation.FuncAnimation(fig, update, frames=len(leg_list), interval=1,)
	#ani.save('limit.gif', writer='pillow', fps=30)

	plt.show()
